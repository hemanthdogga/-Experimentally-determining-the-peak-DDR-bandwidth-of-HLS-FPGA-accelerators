...
//------------------------Parameter----------------------
`define TV_IN_a "D:/VITISplay/using_axi_master/Simulation/tv/cdatafile/c.example.autotvin_a.dat"
`define TV_OUT_a "D:/VITISplay/using_axi_master/Simulation/tv/rtldatafile/rtl.example.autotvout_a.dat"
 parameter a_ADDR_BITWIDTH = 32'd 64;
...
reg    BRESP_tmp = 0;
reg    [a_ID_BITWIDTH - 1 : 0] BID_tmp = 0;
...
reg [a_DATA_BITWIDTH - 1 : 0] a_mem_2 [0: a_mem_depth - 1] = '{default : 'h0}; 
reg [31 : 0] clk_counter ;
reg [31 : 0] current_AW_req_transaction = 0 ;
reg [31 : 0] current_AR_req_transaction = -1 ;
reg write_processing = 0;
reg [a_ADDR_BITWIDTH - 1 : 0] wr_processing_addr = 0;
reg [a_ADDR_BITWIDTH - 1 : 0] rd_processing_addr = 0;
reg [31 : 0] wr_processing_trans_num = 0;
reg [a_DATA_BITWIDTH - 1 : 0] wr_processing_data = 0;
...
always @ (posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        if(TRAN_a_AWREADY & TRAN_a_AWVALID ) begin    // RTL is sending a AW_request to the bus         
            ...
            FIFO_AW_req_ID[FIFO_AW_req_ptr_w]    =   TRAN_a_AWID; 
...
always @ (posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        if(TRAN_a_ARREADY & TRAN_a_ARVALID ) begin    // RTL is sending a AR_request to the bus         
            ...
            FIFO_AR_req_ID[FIFO_AR_req_ptr_w]    =   TRAN_a_ARID; 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_a_AWVALID && TRAN_a_AWREADY && ((FIFO_AW_req_ptr_w - FIFO_AW_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AW_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_a_ARVALID && TRAN_a_ARREADY && ((FIFO_AR_req_ptr_w - FIFO_AR_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AR_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_a_WVALID && TRAN_a_WREADY && ((FIFO_WDATA_req_ptr_w - FIFO_WDATA_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_a_WVALID && TRAN_a_WREADY && TRAN_a_WLAST && ((FIFO_WDATA_size_ptr_w - FIFO_WDATA_size_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        end else if (send_one_BRESP != 'b1 && (BRESP_counter != 32'h0 && TRAN_a_BREADY)) begin
            BRESP_counter <= BRESP_counter - 1;
        end
    end
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > a_mem_depth) begin
                $display ("D:/VITISplay/using_axi_master/vitis1/solution1/sim/verilog/AESL_axi_master_a.v: Write request address %d exceed AXI master a array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, a_mem_depth); 
                $finish;
            end
            ...
            if (FIFO_AW_req_BURST_tmp === 1) begin
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        0 : WDATA_tmp[j] = a_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                                        1 : WDATA_tmp[j] = a_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                                        2 : WDATA_tmp[j] = a_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            0 : a_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            1 : a_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            2 : a_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
                    ...
                    if (FIFO_WDATA_req_empty != 1) begin 
                        ...
                        for (i = 0 ; i < data_byte_size; i = i + 1) begin
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        0 : WDATA_tmp[j] = a_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        1 : WDATA_tmp[j] = a_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        2 : WDATA_tmp[j] = a_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        default: $display("The page_num of AXI write is not valid!");
                                    endcase
                                end
                            end
                        ...
                        wr_processing_data = WDATA_tmp;
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            0 : a_mem_0[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            1 : a_mem_1[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            2 : a_mem_2[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > a_mem_depth) begin
                $display ("D:/VITISplay/using_axi_master/vitis1/solution1/sim/verilog/AESL_axi_master_a.v: Read request address %d exceed AXI master a array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, a_mem_depth); 
                $finish;
            end
            ...
            if (FIFO_AR_req_BURST_tmp === 1) begin
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            0 : RDATA_tmp <= a_mem_0[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                            1 : RDATA_tmp <= a_mem_1[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            0 : RDATA_tmp <= a_mem_0[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            1 : RDATA_tmp <= a_mem_1[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            2 : RDATA_tmp <= a_mem_2[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            default: $display("The page_num of AXI read is not valid!");
                        endcase
                    RVALID_tmp <= 1;
                    //added code end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < a_mem_depth; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  case(mem_page)
                      0 : a_mem_0[i/factor] = mem_tmp;
                      1 : a_mem_1[i/factor] = mem_tmp;
                      2 : a_mem_2[i/factor] = mem_tmp;
...
initial begin : write_file_proc 
  ...
  reg [a_DATA_BITWIDTH - 1 : 0] tmp_data;
  reg [63:0] bin_data;
  transaction_num = 0; 
  a_bitwidth = a_C_DATA_BITWIDTH; 
  count_seperate_factor_by_bitwidth (a_bitwidth , factor);
  while(1) begin 
      ...
      mem_page = transaction_num % mem_page_num ;
      # 0.1;
      fp = $fopen(`TV_OUT_a, "ab"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_a); 
          $finish; 
      end 
      bin_data = a_mem_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < (a_mem_depth - a_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  0 : tmp_data = a_mem_0[i/factor];
                  1 : tmp_data = a_mem_1[i/factor];
                  2 : tmp_data = a_mem_2[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              0 : tmp_data = a_mem_0[a_mem_depth/factor];
              1 : tmp_data = a_mem_1[a_mem_depth/factor];
              2 : tmp_data = a_mem_2[a_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((a_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : tmp_data = a_mem_0[a_mem_depth/factor][15:0];
                  1 : tmp_data = a_mem_1[a_mem_depth/factor][15:0];
                  2 : tmp_data = a_mem_2[a_mem_depth/factor][15:0];
                  default: $display("The page_num of write file is not valid!");
              endcase
              bin_data = tmp_data;
              write_binary(fp,bin_data,16);
